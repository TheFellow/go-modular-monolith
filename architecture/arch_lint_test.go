...
func TestArchitectureRulesRejectForbiddenImports(t *testing.T) {
	...
	testutil.ErrorIf(t, err != nil, "get working directory: %v", err)
	{
		err := os.Chdir(fixtureRoot)
		testutil.ErrorIf(t, err != nil, "enter fixture module: %v", err)
	}
	t.Cleanup(func() {
	...
	testutil.ErrorIf(t, err != nil, "load architecture config: %v", err)
	violations, err := linter.Run(cfg)
	testutil.ErrorIf(t, err != nil, "run arch-lint against fixture module: %v", err)
	...
	slices.Sort(got)
	slices.Sort(want)
	testutil.ErrorIf(t, !slices.Equal(got, want), "unexpected architecture violations:\n got: %q\nwant: %q", got, want)
}

func repositoryRoot(t *testing.T) string {
	t.Helper()
	_, filename, _, ok := runtime.Caller(0)
	testutil.ErrorIf(t, !ok, "%v", "locate architecture test source")
...
func writeFixture(t *testing.T, root, name, contents string) {
	...
	path := filepath.Join(root, name)
	{
		...
		testutil.ErrorIf(t, err != nil, "create fixture directory: %v", err)
	}
	{
		err := os.WriteFile(path, []byte(contents), 0o600)
		testutil.ErrorIf(t, err != nil, "write fixture %s: %v", name, err)
	}
}
