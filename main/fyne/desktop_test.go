...
import (
	...
	drinksfyne "github.com/TheFellow/go-modular-monolith/app/domains/drinks/surfaces/fyne"
	...
	menusfyne "github.com/TheFellow/go-modular-monolith/app/domains/menus/surfaces/fyne"
	ordersfyne "github.com/TheFellow/go-modular-monolith/app/domains/orders/surfaces/fyne"
...
func TestDesktopRoutesBuildConcreteDomainViewsAndActivationReadsCurrentData(t *testing.T) {
	...
	wantTypes := map[string]any{
		"drinks": (*drinksfyne.View)(nil), "ingredients": (*ingredientsfyne.View)(nil),
		"inventory": (*inventoryfyne.View)(nil), "menus": (*menusfyne.View)(nil),
		"orders": (*ordersfyne.View)(nil), "audit": (*auditfyne.View)(nil),
		"tags": (*taggingfyne.View)(nil),
	}
	...
	presenter := desktop.presenters["ingredients"].(*ingredientsfyne.Presenter)
...
func TestDesktopSemanticControlsExerciseEveryWorkspaceAgainstOneSession(t *testing.T) {
	...
	refreshes := []struct{ route, control string }{
		{"drinks", drinksfyne.ControlRefresh},
		{"ingredients", "ingredients-refresh"},
		{"inventory", "inventory-refresh"},
		{"menus", menusfyne.ControlRefresh},
		{"orders", "orders-refresh"},
		{"audit", auditfyne.ControlRefresh},
	}
	for _, item := range refreshes {
		t.Run(item.route, func(t *testing.T) {
			...
			switch item.route {
			case "drinks":
				if state := desktop.presenters[item.route].(*drinksfyne.Presenter).State(); state.Loading || state.Err != nil {
					t.Fatalf("drinks refresh = %#v", state)
				}
			case "ingredients":
				if state := desktop.presenters[item.route].(*ingredientsfyne.Presenter).Snapshot(); state.Status != fyneui.Loaded || state.Err != nil {
					t.Fatalf("ingredients refresh = %#v", state)
				}
			case "inventory":
				if state := desktop.presenters[item.route].(*inventoryfyne.Presenter).Snapshot(); state.Status != fyneui.Loaded || state.Err != nil {
					t.Fatalf("inventory refresh = %#v", state)
				}
			case "menus":
				if state := desktop.presenters[item.route].(*menusfyne.Presenter).State(); state.Loading || state.Err != nil {
					t.Fatalf("menus refresh = %#v", state)
				}
			case "orders":
				if state := desktop.presenters[item.route].(*ordersfyne.Presenter).State(); state.Loading || state.Err != nil {
					t.Fatalf("orders refresh = %#v", state)
				}
			case "audit":
				if state := desktop.presenters[item.route].(*auditfyne.Presenter).State(); state.Loading || state.Err != nil {
	...
	driver.Tap(taggingfyne.ControlSummary)
	if state := desktop.presenters["tags"].(*taggingfyne.Presenter).State(); state.Mode != taggingfyne.Results || state.Err != nil {
...
func TestRestrictedDesktopSemanticMutationShowsTypedDialogAndDoesNotWrite(t *testing.T) {
	...
	driver.Type(taggingfyne.ControlValue, "denied=yes")
	...
	if state := desktop.presenters["tags"].(*taggingfyne.Presenter).State(); state.Mode != taggingfyne.EnteringValue || state.Err == nil {
...
func TestDesktopMenuShortcutsNavigateAndRespectWorkspaceMode(t *testing.T) {
	...
	presenter := desktop.presenters["drinks"].(*drinksfyne.Presenter)
	file := menu.Items[1]
	file.Items[0].Action()
	if presenter.State().Mode != drinksfyne.Creating {
	...
	file.Items[4].Action()
	if presenter.State().Mode != drinksfyne.Browsing {
