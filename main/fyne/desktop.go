...
func openDesktopWithDependencies(ctx context.Context, gui framework.App, config desktopConfig, deps desktopDependencies) (*desktop, error) {
	...
	routes := []fyneui.Route{
		...
		{ID: "drinks", Label: "Drinks", Build: owned("drinks", func() fyneui.View {
			presenter := drinksfyne.NewPresenter(d.session, drinksfyne.Dependencies{Executor: deps.executor, Dispatcher: deps.dispatcher, Dialogs: dialogs()})
			d.presenters["drinks"] = presenter
			return drinksfyne.NewView(presenter)
		})},
		{ID: "ingredients", Label: "Ingredients", Build: owned("ingredients", func() fyneui.View {
			presenter := ingredientsfyne.NewPresenter(d.session, deps.executor, deps.dispatcher, dialogs())
			d.presenters["ingredients"] = presenter
			return ingredientsfyne.NewView(presenter)
		})},
		{ID: "inventory", Label: "Inventory", Build: owned("inventory", func() fyneui.View {
			presenter := inventoryfyne.NewPresenter(d.session, deps.executor, deps.dispatcher, dialogs())
			d.presenters["inventory"] = presenter
			return inventoryfyne.NewView(presenter)
		})},
		{ID: "menus", Label: "Menus", Build: owned("menus", func() fyneui.View {
			presenter := menusfyne.NewPresenter(d.session, menusfyne.Dependencies{Executor: deps.executor, Dispatcher: deps.dispatcher, Dialogs: dialogs()})
			d.presenters["menus"] = presenter
			return menusfyne.NewView(presenter)
		})},
		{ID: "orders", Label: "Orders", Build: owned("orders", func() fyneui.View {
			presenter := ordersfyne.NewPresenter(d.session, ordersfyne.Dependencies{Executor: deps.executor, Dispatcher: deps.dispatcher, Dialogs: dialogs()})
			d.presenters["orders"] = presenter
			return ordersfyne.NewView(presenter)
		})},
		{ID: "audit", Label: "Audit", Build: owned("audit", func() fyneui.View {
			presenter := auditfyne.NewPresenter(d.session, auditfyne.Dependencies{Executor: deps.executor, Dispatcher: deps.dispatcher, Dialogs: dialogs()})
			d.presenters["audit"] = presenter
			return auditfyne.NewView(presenter)
		})},
		{ID: "tags", Label: "Tags", Build: owned("tags", func() fyneui.View {
			presenter := taggingfyne.NewPresenter(d.session, taggingfyne.Dependencies{Executor: deps.executor, Dispatcher: deps.dispatcher, Dialogs: dialogs()})
			d.presenters["tags"] = presenter
			return taggingfyne.NewView(presenter)
		})},
	}
