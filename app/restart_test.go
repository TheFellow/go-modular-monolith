...
func TestApp_RestartPreservesCompletedOrderWorkflowAndAudit(t *testing.T) {
	...
	testutil.Ok(t, err)
	testutil.Equals(t, depletedStock.Amount, measurement.MustAmount(0, ingredient.Unit), testutil.EquateAmounts(0.000001))
	...
	testutil.Ok(t, err)
	testutil.Equals(t, gotStock, depletedStock, testutil.EquateAmounts(0.000001))
	gotDrink, err := reopened.Drinks.Get(reopenedCtx, drink.ID)
	testutil.Ok(t, err)
	testutil.Equals(t, gotDrink, drink, testutil.EquateAmounts(0.000001), cmpopts.EquateEmpty())
