...
func TestTaggedMenuTransitionsReplaceClearPreserveAndRejectInvalidAtomically(t *testing.T) {
	...
	p.Refresh()
	p.Publish()
	dialogs.TaggedConfirmations()[0].Respond(true, appfyne.ReplaceTags, "featured,region=west")
	...
	p.ReturnToDraft()
	dialogs.TaggedConfirmations()[1].Respond(true, appfyne.ClearTags, "ignored")
	stored, err = f.Menus.Get(f.OwnerContext(), menu.ID)
	testutil.Ok(t, err)
	...
	dialogs.TaggedConfirmations()[2].Respond(true, appfyne.PreserveTags, "ignored")
	stored, err = f.Menus.Get(f.OwnerContext(), menu.ID)
	testutil.Ok(t, err)
	...
	p.Publish()
	dialogs.TaggedConfirmations()[3].Respond(true, appfyne.ReplaceTags, "region=east,region=west")
	stored, err = f.Menus.Get(f.OwnerContext(), menu.ID)
	testutil.Ok(t, err)
	testutil.Equals(t, stored.Status, models.MenuStatusDraft)
...
func TestDeniedMutationRetainsFormAndRollsBack(t *testing.T) {
	...
	dialogs := &fynetest.Dialogs{}
	p := NewPresenter(denied, Dependencies{Executor: appfyne.InlineExecutor{}, Dispatcher: appfyne.InlineDispatcher{}, Dialogs: dialogs})
	p.state.Items = []*models.Menu{menu}
	p.Select(0)
	p.StartRename()
...
func TestAcceptedSubmissionAndConfirmationDisableEveryControl(t *testing.T) {
	...
	dialogs := &fynetest.Dialogs{}
	p := NewPresenter(f.App, Dependencies{Executor: executor, Dispatcher: appfyne.InlineDispatcher{}, Dialogs: dialogs})
	p.state.Items = []*models.Menu{menu}
	p.Select(0)
	v := NewView(p)
	p.StartRename()
