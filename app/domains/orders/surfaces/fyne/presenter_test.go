...
func TestTaggedTerminalConfirmationsReplaceClearPreserveAndRejectInvalidAtomically(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		mode                           appfyne.TagMutationMode
		wantStatus                     models.OrderStatus
	}{
		{"complete replace", "complete", "featured,region=west", "featured,region=west", appfyne.ReplaceTags, models.OrderStatusCompleted},
		{"cancel clear", "cancel", "ignored", "", appfyne.ClearTags, models.OrderStatusCancelled},
		{"cancel preserve", "cancel", "ignored", "old", appfyne.PreserveTags, models.OrderStatusCancelled},
	} {
	...
	p.ConfirmComplete()
	dialogs.TaggedConfirmations()[0].Respond(true, appfyne.ReplaceTags, "region=east,region=west")
	stored, err := f.Orders.Get(f.OwnerContext(), order.ID)
	testutil.Ok(t, err)
	testutil.Equals(t, stored.Status, models.OrderStatusPending)
...
func TestTagValidationAndAuthorizationFailuresRetainCorrectableForms(t *testing.T) {
	...
	p = NewPresenter(denied, Dependencies{Executor: appfyne.InlineExecutor{}, Dispatcher: appfyne.InlineDispatcher{}, Dialogs: &fynetest.Dialogs{}})
	p.Refresh()
	p.StartTags()
...
func TestCatalogRefreshDisablesEveryPlacementControl(t *testing.T) {
	...
	p := NewPresenter(f.App, Dependencies{Executor: executor, Dispatcher: appfyne.InlineDispatcher{}, Dialogs: &fynetest.Dialogs{}})
	v := NewView(p)
	p.StartPlace()
	testutil.Equals(t, executor.RunNext(), true)
	p.ChooseMenu(menu.ID)
...
func TestCLIAndFyneShareOrderPersistenceContract(t *testing.T) {
	...
	core = application.New(ctx, application.Config{Store: database})
	dialogs := &fynetest.Dialogs{}
	p := NewPresenter(application.NewSession(ctx, core), Dependencies{Executor: appfyne.InlineExecutor{}, Dispatcher: appfyne.InlineDispatcher{}, Dialogs: dialogs})
	p.Refresh()
...
func newInlinePresenter(f *testutil.Fixture) *Presenter {
	return NewPresenter(f.App, Dependencies{Executor: appfyne.InlineExecutor{}, Dispatcher: appfyne.InlineDispatcher{}, Dialogs: &fynetest.Dialogs{}})
}
