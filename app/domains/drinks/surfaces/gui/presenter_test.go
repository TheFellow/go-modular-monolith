...
func TestRetiredRecipeRemainsReadableAndCanBeRepaired(t *testing.T) {
	a := test.NewApp()
	defer a.Quit()
	f, retired, drink := fixtureDrink(t, "Needs review")
	...
	p.Refresh()
	p.Select(0)
	testutil.StringContains(t, v.recipeStatus.Text, "Status: review_required")
	testutil.StringContains(t, v.recipeStatus.Text, "Recipe requires review")
	testutil.StringContains(t, v.recipe[0].ingredient.Selected, "Retired or missing ingredient")
	testutil.StringContains(t, v.recipe[0].ingredient.Selected, retired.ID.String())
	testutil.Equals(t, v.recipe[0].ingredient.Visible(), true)
	// An unrelated edit must preserve the unavailable reference until repaired.
	driver.Type(ControlName, "Repairing recipe")
	testutil.Equals(t, p.State().Form.Recipe[0].Ingredient, retired.ID)
	v.recipe[0].ingredient.SetSelected(v.optionLabel(replacement.ID))
	driver.Tap(ControlSave)
	testutil.Ok(t, p.State().Err)
	got, err := f.Drinks.Get(f.OwnerContext(), drink.ID)
	testutil.Ok(t, err)
	testutil.Equals(t, got.Status, models.StatusActive)
	testutil.Equals(t, got.Recipe.Ingredients[0].IngredientID, replacement.ID)
	testutil.Equals(t, got.Recipe.Steps, []string{"Stir"})
	testutil.StringContains(t, v.recipeStatus.Text, "Status: active")
}

func TestDeleteExplainsMenuDependencyAndSurfacesVeto(t *testing.T) {
	f, _, drink := fixtureDrink(t, "Menu dependency")
	menu := testutil.CreateMenu(t, f, "Keep curated", testutil.WithDrink(drink))
	dialogs := &fynetest.Dialogs{}
	p := NewPresenter(f.App, Dependencies{Executor: appgui.InlineExecutor{}, Dispatcher: appgui.InlineDispatcher{}, Dialogs: dialogs})
	p.Refresh()
	p.Select(0)
	...
	confirmations := dialogs.Confirmations()
	testutil.Equals(t, len(confirmations), 1)
	testutil.StringContains(t, confirmations[0].Message, "cannot be deleted until it is removed from every menu")
	testutil.StringContains(t, confirmations[0].Message, "Order usage also prevents deletion")
	confirmations[0].Respond(true)
	testutil.ErrorIsFailedPrecondition(t, p.State().Err)
	testutil.StringContains(t, p.State().Err.Error(), menu.Name)
	testutil.Equals(t, len(dialogs.Warnings()), 1)
	testutil.StringContains(t, dialogs.Warnings()[0].Message, menu.Name)
	_, err := f.Drinks.Get(f.OwnerContext(), drink.ID)
	testutil.Ok(t, err)
}
