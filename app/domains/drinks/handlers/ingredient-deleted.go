...
func (h *IngredientDeleted) Handle(ctx *middleware.HandlerContext, _e ingredientsevents.IngredientDeleted) error {
	...
	for _, drink := range h.affectedDrinks {
		...
		unit := _e.Ingredient.Unit
		if _e.Replacement != nil {
			replacement = _e.Replacement.ID
			unit = _e.Replacement.Unit
		}
		review, err := drink.RetireIngredient(_e.Ingredient.ID, replacement, unit, _e.ReplacementRatio)
		if err != nil {
			return err
		}
		...
		ctx.RecordEffect("recipe_rewritten", review.ID.EntityUID(), middlewareevents.Change{Field: "ingredients", Before: fmt.Sprint(drink.Recipe.Ingredients), After: fmt.Sprint(review.Recipe.Ingredients)})
	}
	return nil
}
