...
func (m *ListViewModel) Update(msg tea.Msg) (tui.ViewModel, tea.Cmd) {
	...
	case tui.DataInvalidatedMsg:
		if m.mode != listModeBrowsing || !m.actionEnabled(inventory.ControlList) {
			return m, nil
		}
...
func (m *ListViewModel) startTags() tea.Cmd {
	...
	m.tags = components.NewTagEditor(m.app.TagReplacer(row.Inventory.Tags), tag.ParseCollection, row.Inventory.EntityUID(), row.Ingredient.Name, row.Inventory.Tags.Canonical().String())
	m.tags.SetWidth(m.width)
	return m.tags.Init()
}
