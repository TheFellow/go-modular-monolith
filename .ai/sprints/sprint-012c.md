# Sprint 012c: Architectural Lint Rules (Intermezzo)
...
Handlers react to events from OTHER modules. They should not import their own module's events (which would suggest emitting them).
...
- [ ] Add `arch-lint` as a tool dependency in `go.mod`
- [ ] Create `.arch-lint.yaml` with above rules
- [ ] Verify current code passes all rules
- [ ] Add arch-lint to CI pipeline
...
specs:
  ...
  # Handlers handle events from OTHER modules, not their own
  - name: handlers-no-own-events
    packages:
      include:
        - "app/{module}/handlers/**"
