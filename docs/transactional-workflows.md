...
CLI examples (replace identifiers with real IDs):
...
Batch input is a JSON array of `orders/models.Amendment`, including each expected revision. The CLI exposes amendment and inventory lifecycle commands; GUI/TUI historical details show approved preparation and amendment reasons. New workflow forms are not required to invoke the public application contracts.
