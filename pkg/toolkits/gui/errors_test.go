...
func TestPresentErrorClassifiesKindsAndProtectsInternalDetail(t *testing.T) {
	...
	tests := []struct {
		...
		{"invalid", errors.Invalidf("name is required"), gui.ErrorSeverityInline, "name is required"},
		{"not found", errors.NotFoundf("drink missing"), gui.ErrorSeverityWarning, "drink missing"},
		{"permission", errors.Permissionf("may not edit"), gui.ErrorSeverityError, "may not edit"},
		{"conflict", errors.Conflictf("name already exists"), gui.ErrorSeverityWarning, "name already exists"},
		{"precondition", errors.FailedPreconditionf("menu is published"), gui.ErrorSeverityWarning, "menu is published"},
		{"internal", errors.Internalf("database password leaked"), gui.ErrorSeverityError, "internal error"},
...
func TestPresentErrorHandlesNilWrappedAndExplicitSafeMessages(t *testing.T) {
	...
	cause := errors.Conflictf("diagnostic detail").WithUserMessage("That name is already in use")
...
func TestShowPresentationUsesSeverityAndSuppressesInlineValidation(t *testing.T) {
	...
	gui.ShowPresentation(dialogs, errors.Invalidf("fix field"))
	gui.ShowPresentation(dialogs, errors.Conflictf("already exists"))
	permission := errors.Permissionf("denied")
