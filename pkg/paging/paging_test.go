...
func TestCollectTraversesPages(t *testing.T) {
	t.Parallel()

	items, err := paging.Collect(func(cursor paging.Cursor) (paging.Page[int], error) {
		page := 0
		if cursor != "" {
			var err error
			page, err = strconv.Atoi(string(cursor))
	...
	testutil.Ok(t, err)
	testutil.Equals(t, items, []int{1, 2, 3, 4, 5})
}
